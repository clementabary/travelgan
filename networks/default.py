...
class Generator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, dropout=False,
        ...
        for i in range(num_downs - 5):
            unet_block = UnetSkipConnectionBlock(
                ngf * 8, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer, use_dropout=dropout)
...
class Siamese(nn.Module):
    ...
    def __init__(self, input_nc, nsf, n_layers, latent_dim,
        ...
        kw = 4
        padw = 1
        sequence = [nn.Conv2d(input_nc, nsf, kernel_size=kw,
                              stride=2, padding=padw), nn.LeakyReLU(0.2, True)]
        nf_mult = 1
        nf_mult_prev = 1
        for n in range(1, n_layers):
            ...
            nf_mult = min(2 ** n, 8)
            sequence += [
                nn.Conv2d(nsf * nf_mult_prev, nsf * nf_mult,
                          kernel_size=kw, stride=2, padding=padw, bias=use_bias),
                norm_layer(nsf * nf_mult),
                nn.LeakyReLU(0.2, True)
            ]

        # output 1 channel prediction map
        sequence += [nn.Conv2d(nsf * nf_mult, latent_dim,
                               kernel_size=2**(5-n_layers), stride=1)]
        self.model = nn.Sequential(*sequence)
